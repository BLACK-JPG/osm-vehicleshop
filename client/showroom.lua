...
AddEventHandler('qb-vehicleshop:client:DoTestrit', function(plate)
    if ClosestVehicle ~= 0 then
        QBCore.Functions.SpawnVehicle(QBCustom.ShowroomPositions[ClosestVehicle].vehicle, function(veh)
            ...
            QBCore.Functions.Notify('You have 3 minutes for test drive', 'success', 3500)

            SetTimeout(QB.PDMTestDrive * 60000, function()
                if testritveh ~= 0 then
                    testritveh = 0
                    QBCore.Functions.DeleteVehicle(veh)
                    QBCore.Functions.Notify('It is 3 minutes, do you like it?', 'error', 3500)
                end
            end)
