...
AddEventHandler('qb-vehicleshop:server:buyVehicle', function(vehicleData, garage)
    ...
    if (balance - vData["price"]) >= 0 then
        ...
        exports.oxmysql:insert("INSERT INTO `player_vehicles` (`license`, `citizenid`, `vehicle`, `hash`, `mods`, `plate`, `garage`) VALUES ('"..pData.PlayerData.license.."', '"..cid.."', '"..vData["model"].."', '"..GetHashKey(vData["model"]).."', '{}', '"..plate.."', '"..garage.."')")
...
AddEventHandler('qb-vehicleshop:server:FinanceVehicle', function(vehicle, class, finserv, downp)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    local vehiclePrice = QBCore.Shared.Vehicles[vehicle]["price"]
    local vehicleValue = ((QBCore.Shared.Vehicles[vehicle]["price"] - downp))

    exports.oxmysql:fetch("SELECT * FROM `players` WHERE `citizenid` = ? ", {cid}, function(result)
        if result[1] ~= nil then
            for k, v in pairs(result) do
                if v.pending_finance == 0 then
                        if (balance - downp) >= 0 then
                            exports.oxmysql:insert("INSERT INTO `player_vehicles` (`license`, `citizenid`, `vehicle`, `hash`, `mods`, `plate`, `state`,`finance_owed`, `finperc`) VALUES ('"..pData.PlayerData.license.."', '"..cid.."', '"..vehicle.."', '"..GetHashKey(vehicle).."', '{}', '"..plate.."', 0, '"..vehicleValue.."', '"..finserv.."')")
                            exports.oxmysql:execute("UPDATE `players` SET `pending_finance` = ? WHERE `citizenid` = ?", {1, cid})
                            TriggerClientEvent("QBCore:Notify", src, "Successful! Your vehicle is waiting for you outside.", "success", 5000)
                            TriggerEvent('qb-phone:server:sendNewMailToOffline', Player.PlayerData.citizenid, {
                                sender = "PDM Vehicle Shop",
                                subject = "Car receipt",
...
print("Cutting finances")
    exports.oxmysql:fetch("SELECT * FROM `player_vehicles`", {}, function(result)
        if result[1] ~= nil then
            for k, v in pairs(result) do
                	...
                	if Player ~= nil then
	                    if v.finance_owed > 0 then
	                        ...
	                        if (balance - installment >= 0) then
	                            exports.oxmysql:execute("UPDATE `player_vehicles` SET `finance_owed` = '"..v.finance_owed.."' - '"..installment.."' WHERE `plate` = '"..v.plate.."'")
	                    ...
	                    elseif v.finance_owed < 0 then
	                        exports.oxmysql:execute("UPDATE `player_vehicles` SET `finance_owed` = 0 WHERE `plate` = '"..v.plate.."'")
                            exports.oxmysql:execute("UPDATE `players` SET `pending_finance` = 0 WHERE `citizenid` = '"..v.citizenid.."'")
	                    end
                    else
                        exports.oxmysql:fetch("SELECT * FROM `players` WHERE `citizenid` = '"..v.citizenid.."'", {}, function(result)
                            if result[1] ~= nil then
                                if finowd > 0 then
                                    local multiplier = v.finperc / 100
                                    ...
                                    if NewBal.bank >= 0 then
                                        exports.oxmysql:execute("UPDATE `players` SET `money` = '"..json.encode(NewBal).."' WHERE `citizenid` = '"..v.citizenid.."'")
                                        exports.oxmysql:execute("UPDATE `player_vehicles` SET `finance_owed` = '"..v.finance_owed.."' - '"..installment.."' WHERE `plate` = '"..v.plate.."'")
                                    end
        ...
        if result[1] ~= nil then
            for k, v in pairs(result) do
                if v.pending_finance == 0 then
                    ...
                    exports.oxmysql:fetch("SELECT * FROM `player_vehicles` WHERE `citizenid` = ? ", {cid}, function(result)
                        if result[1] ~= nil then
                        for k, v in pairs(result) do
                            ...
                            if v.finance_owed > 0 then
                                if (amount - v.finance_owed) < 0 then
                                            if (balance >= amount) then
                                                exports.oxmysql:execute("UPDATE `player_vehicles` SET `finance_owed` = '"..v.finance_owed.."' - '"..amount.."' WHERE `plate` = '"..v.plate.."'")
                                ...
                                elseif (amount - v.finance_owed) == 0 then
                                    print(8)
                                            if (balance >= amount) then
                                                ...
                                                Player.Functions.RemoveMoney("bank",amount,'paid-finance')
                                                TriggerClientEvent('chatMessage', source, "SYSTEM", "success", 'You paid $'..amount..' against pending finances.')
                                                exports.oxmysql:execute("UPDATE `players` SET `pending_finance` = 0 WHERE `citizenid` = '"..cid.."'")
                                            else
                                                TriggerClientEvent('chatMessage', source, "SYSTEM", "error", 'You don\'t have enough money.')
                                            end
                                else 
...
AddEventHandler('qb-vehicleshop:server:ConfirmVehicle', function(ShowroomVehicle)
    ...
    if Player.PlayerData.money.cash >= VehPrice then
        ...
        exports.oxmysql:insert("INSERT INTO `player_vehicles` (`license`, `citizenid`, `vehicle`, `hash`, `mods`, `plate`, `state`) VALUES ('"..Player.PlayerData.license.."', '"..Player.PlayerData.citizenid.."', '"..ShowroomVehicle.vehicle.."', '"..GetHashKey(ShowroomVehicle.vehicle).."', '{}', '"..plate.."', 0)")
...
AddEventHandler('qb-vehicleshop:server:ConfirmVehicleFinance', function(vehicle, class)
    ...
    if (balance - financeInstallment) >= 0 then
        exports.oxmysql:insert("INSERT INTO `player_vehicles` (`license`, `citizenid`, `vehicle`, `hash`, `mods`, `plate`, `state`,`finance_owed`) VALUES ('"..pData.PlayerData.license.."', '"..cid.."', '"..ShowroomVehicle.vehicle.."', '"..GetHashKey(ShowroomVehicle.vehicle).."', '{}', '"..plate.."', 0, '"..vehicleValue.."')")
        TriggerClientEvent("QBCore:Notify", src, "Successful! Your vehicle is waiting for you outside.", "success", 5000)
        TriggerClientEvent('qb-vehicleshop:client:ConfirmVehicle', src, ShowroomVehicle, plate)
...
QBCore.Functions.CreateCallback('qb-vehicleshop:server:SellVehicle', function(source, cb, vehicle, plate)
    ...
    exports.oxmysql:fetch("SELECT * FROM `player_vehicles` WHERE `citizenid` = '"..Player.PlayerData.citizenid.."' AND `plate` = '"..plate.."'", function(result)
        if result[1] ~= nil then
            if result[1].finance_owed <= 0 then
                Player.Functions.AddMoney('bank', math.ceil(VehicleData["price"] / 100 * 60))
                exports.oxmysql:execute("DELETE FROM `player_vehicles` WHERE `citizenid` = '"..Player.PlayerData.citizenid.."' AND `plate` = '"..plate.."'")
                cb(true)
            else
